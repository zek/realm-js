    ...
    Object.defineProperties(realmConstructor, getOwnPropertyDescriptors({
        open(config) {
            return new Promise((resolve, reject) => {
                this.openAsync(config, (error, realm) => {
                    if (error) {
                        reject(error);
                    } else {
                        resolve(realm);
        ...
        openAsync(config, callback) {
            function openRealm(config, callback) {
                let realm, error;
                try {
                    realm = new this(config);
                } catch (e) {
                    error = e;
                }
            ...
            if ('sync' in config) {
                let session = this.Sync.getOrCreateSession(config.sync, config.path);
                session.waitForDownload((error) => {
                    if (error) {
                        callback(error);
                    } else {
                        openRealm.call(this, config, callback);
                    }
                })
            } else {
                openRealm.call(this, config, callback);
